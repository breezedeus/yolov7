...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.stride = stride
        self.path = path
        self.albumentations = Albumentations([self.img_size, self.img_size]) if augment else None
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            if random.random() <= 1.0 - hyp.get('mosaic9', 0.0):
        ...
        if self.augment:
            ...
            # plot_boxes(img, labels, categories=[0, 1], save_path=f'check-{index}.png')

            img, labels = self.albumentations(img, labels)

            # plot_boxes(img, labels, categories=[0, 1], save_path=f'check-{index}-after.png')
            # breakpoint()
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if n:
        ...
        targets[:, [1, 3]] = np.clip(targets[:, [1, 3]], 0, img.shape[1])
        targets[:, [2, 4]] = np.clip(targets[:, [2, 4]], 0, img.shape[0])
        # 去掉大小为0的boxes
        targets = targets[targets[:, 1] < targets[:, 3], :]
        targets = targets[targets[:, 2] < targets[:, 4], :]
    return img, targets
...
class Albumentations:
    ...
    def __init__(self, img_size):
        ...
        self.transform = A.Compose([
            # A.RandomCropNearBBox(max_part_shift=(0.1, 0.5), cropping_box_key=''),
            # A.RandomSizedCrop(min_max_height=(200, img_size[0]), height=img_size[0], width=img_size[1], p=1.0),
            A.CropAndPad(percent=((-0.45, 0.0), (-0.4, 0.1), (-0.45, 0.0), (-0.4, 0.1)), pad_cval=0, p=0.6),
            A.CLAHE(p=0.01),
            ...
            bbox_params=A.BboxParams(format='pascal_voc', min_area=10, min_visibility=0.4, label_fields=['class_labels']))
